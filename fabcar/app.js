...
app.post('/api/car', function (req, res) {
	...
	Fabric_Client.newDefaultKeyValueStore({
		...
		var request = {
			chaincodeId: 'fabcar',
			fcn: (req.body.isnew == 'true' ? 'createCar' : 'changeCar'),
			args: [req.body.key, req.body.owner, req.body.make, req.body.model, req.body.colour],
			chainId: 'mychannel',
			txId: tx_id
		};
		console.log(request);
		...
		if (isProposalGood) {
			...
			console.log('transactionid:' + transaction_id_string + ',peer: ' + JSON.stringify(peer))
		...
		//channel.queryInfo(peer).then((Blockchain) => {
		channel.queryBlockByTxID(results[1].tx_id).then((Block) => {
